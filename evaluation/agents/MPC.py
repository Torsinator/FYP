...
def create_lander_model():
    ...
    model.set_rhs('x0', x2)
    model.set_rhs('x1', x3)
    F_body_x = u1 * SIDE_POWER
    F_body_y = u0 * MAIN_POWER

    F_world_x = ca.cos(x4) * F_body_x - ca.sin(x4) * F_body_y
    F_world_y = ca.sin(x4) * F_body_x + ca.cos(x4) * F_body_y

    model.set_rhs('x2', F_world_x / m)
    model.set_rhs('x3', F_world_y / m + GRAVITY)  # assuming g = 10 m/s²
    model.set_rhs('x4', x5)
    model.set_rhs('x5', -h / I * F_body_x)
...
class MPC():
    ...
    def setup(self, obs):
        ...
        mterm = weights[2] * (target_state[2] - model.x['x4'])**2
