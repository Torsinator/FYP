...
class Minimum_Distance_Shaped_Cost_SINDy():
    def __init__(self, gym_env, ode_file):
        self.env = gym_env
        self.last_target = np.array([])
        self.ode_file = ode_file
    ...
    def load_model(cls):
        env = envs.LunarLanderMPC.make_env()
        model = Minimum_Distance_Shaped_Cost_SINDy(env, f"models/SINDy.txt")
        return model, env
    
    @classmethod
    def create_model(cls):
        return Minimum_Distance_Shaped_Cost_SINDy.load_model()
...
class OTR_Thresholded():
    ...
    def load_model(cls):
        env = envs.LunarLanderMPC.make_env()
        model = OTR_Thresholded(env, f"models/SINDy.txt")
        return model, env
    
    @classmethod
    def create_model(cls):
        return OTR_Thresholded.load_model()
    ...
    def setup(self, obs):
        def g(dist, k=1.0, eps=50):
            z = k * (eps - dist)
            return 0.5 * (1 + ca.tanh(z))
        # === Setup MPC Controller ===
        print("MPC reset")
        model = create_lander_model(load_rhs_equations(self.ode_file))
        mpc = do_mpc.controller.MPC(model)
        ...
        lterm = (1-gx) * ca.sum1(dists)
    
        mpc.nlp_obj = lterm + (gx * reward_weight * soft_min)
        mpc.create_nlp()
        # mpc.setup()
        mpc.x0 = gym_state_to_mpc(obs)
        mpc.set_initial_guess()
