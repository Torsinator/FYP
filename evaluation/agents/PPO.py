...
class PPO_Unweighted_9_States_Sparse():
    ...
    def load_model(cls):
        ...
        model = PPO.load(f"models/{cls.__name__}", env=env, device="cpu")
        return model, env

class PPO_Unweighted_9_States_Dense():
    @classmethod
    def create_model(cls):
        env = lambda: envs.LunarLander9Obs.make_env(weighted=False, dictionary_obs=False, dense_reward=True)
        # env = make_vec_env(env, num_envs, monitor_dir=get_log_dir(cls.__name__))        
        model = PPO("MlpPolicy", env(), verbose=1, device="cpu")
        return model, env
...
class PPO_Unweighted_6_States_Dense():
    @classmethod
    def create_model(cls):
        env = lambda: envs.LunarLander6Obs.make_env(weighted=False, dictionary_obs=False, dense_reward=True)
        env = make_vec_env(env, num_envs, monitor_dir=get_log_dir(cls.__name__))   
        model = PPO("MlpPolicy", env, verbose=1, device="cpu")
        return model, env
...
class PPO_Weighted_9_States_Sparse():
    @classmethod
    def create_model(cls):
        env = lambda: envs.LunarLander9Obs.make_env(weighted=True, dictionary_obs=False, dense_reward=False)
        env = make_vec_env(env, num_envs, monitor_dir=get_log_dir(cls.__name__))  
        model = PPO("MlpPolicy", env, verbose=1, device="cpu")
        return model, env
...
class PPO_Weighted_9_States_Dense():
    @classmethod
    def create_model(cls):
        env = lambda: envs.LunarLander9Obs.make_env(weighted=True, dictionary_obs=False, dense_reward=True)
        env = make_vec_env(env, num_envs, monitor_dir=get_log_dir(cls.__name__))  
        model = PPO("MlpPolicy", env, verbose=1, device="cpu")
        return model, env
...
class PPO_Weighted_6_States_Sparse():
    @classmethod
    def create_model(cls):
        env = lambda: envs.LunarLander6Obs.make_env(weighted=True, dictionary_obs=False, dense_reward=False)
        env = make_vec_env(env, num_envs, monitor_dir=get_log_dir(cls.__name__))  
        model = PPO("MlpPolicy", env, verbose=1, device="cpu")
        return model, env
...
class PPO_Weighted_6_States_Dense():
    @classmethod
    def create_model(cls):
        env = lambda: envs.LunarLander6Obs.make_env(weighted=True, dictionary_obs=False, dense_reward=True)
        env = make_vec_env(env, num_envs, monitor_dir=get_log_dir(cls.__name__))  
        model = PPO("MlpPolicy", env, verbose=1, device="cpu")
        return model, env
