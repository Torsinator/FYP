    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        if self.continuous:
            ...
            self.action_space = spaces.Box(np.array([0, -1]), np.array([1, 1]), (2,), dtype=np.float32)
        else:
    ...
    def render(self):
        ...
        text_surface = self.font.render(f"Current State: {state_string}", True, (255, 255, 255))  # white text
        self.surf.blit(text_surface, (20, 40))

        text_surface = self.font.render(f"Reward: {self.prev_reward:.2f}", True, (255, 255, 255))  # white text
        self.surf.blit(text_surface, (20, 60))
...
register(
    ...
    entry_point="environments.custom_lunar_lander_no_target:LunarLander",
