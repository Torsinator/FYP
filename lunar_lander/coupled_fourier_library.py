...
class CoupledFourierLibrary(BaseFeatureLibrary):
    """
    ...
    def get_feature_names(self, input_features=None):
        ...
        for i in range(self.n_frequencies):
            for feature in input_features[:self.n_states]:
                for control in range(self.n_controls):
                    if self.include_sin:
                        feature_names.append("u" + str(control) + " sin(" + str(i + 1) + " " + feature + ")")
                    if self.include_cos:
                        feature_names.append("u" + str(control) + " cos(" + str(i + 1) + " " + feature + ")")
    ...
    def fit(self, x_full, y=None):
        ...
        self.n_states = n_features - self.n_controls
        if self.include_sin and self.include_cos:
            self.n_output_features_ = self.n_states  * self.n_frequencies * 2 * self.n_controls
        else:
            self.n_output_features_ = self.n_states * self.n_frequencies * self.n_controls
        return self
            ...
            for i in range(self.n_frequencies):
                for j in range(self.n_states):
                    for k in range(self.n_controls):
                        if self.include_sin:
                            xp[..., idx] = x[..., self.n_states + k] * np.sin((i + 1) * x[..., j])
                            idx += 1
                        if self.include_cos:
                            xp[..., idx] = x[..., self.n_states + k] * np.cos((i + 1) * x[..., j])
                            idx += 1
