...
class CustomEnvironmentWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        "desired_goal": np.array(self.target_state, dtype=np.float32)}, reward, reward == 1 or terminated, truncated, info
        return obs, reward, reward == 1 or terminated, truncated, info
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None) -> tuple[Any, dict[str, Any]]:
        ...
        "observation": obs.astype(np.float32),
        "achieved_goal": np.array(obs[[0,1,2]], dtype=np.float32),
        "desired_goal": np.array(self.target_state, dtype=np.float32)}, info
        return obs, info
    ...
    def compute_reward(
        ...
        return np.where(dist < 0.2, 1.0, 0).astype(np.float32) 
        # return dist