...
class SINDyAgent(Agent):
    ...
    def load(model_path, gym_env=None) -> Agent:
        return SINDyAgent(gym_env, model_path)
    ...
    def setup(self, obs):
        ...
        mterm = weights[0] * (target_state[0] - model.x['x0'])**2 + weights[1] * (target_state[1] - model.x['x1'])**2 + weights[2] * (target_state[2] - model.x['x4'])**2 + model.x['x2']**2 + model.x['x3']**2
        lterm = mterm
        mpc.set_objective(mterm=mterm, lterm=lterm)
        mpc.set_rterm(u0=1e-1, u1=1e-1)
